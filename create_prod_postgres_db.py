...
try:
    ...
    # -- does user exists
    cur.execute("select * from pg_roles where rolname='%s'" % ('prod_user',))
    usr_exists = bool(cur.rowcount) 
 
    if usr_exists == False:
        sqlfile = open('prod_resources/Create prod_user.sql', 'r')
        cur.execute(sqlfile.read())
       
    db_name = "enablement_prod"
    # db_name = "foo" 
    ...
    cur.execute("GRANT TEMPORARY, CONNECT ON DATABASE enablement_prod TO prod_user")
    cur.execute("GRANT ALL ON DATABASE enablement_prod TO postgres")
    ...
    sqlfile = open('prod_resources/Create Global_Superstore Tables prod.sql', 'r')
    cur.execute(sqlfile.read())               
    con.commit();
    
    # finally insert data
    sqlfile = io.open('prod_resources/prod_inserts_utf_8.sql', mode='r', encoding='utf-8-sig')
    cur.execute(sqlfile.read())               
    con.commit();    
